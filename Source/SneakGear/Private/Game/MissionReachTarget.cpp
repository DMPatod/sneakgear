#include "Game/MissionReachTarget.h"
...
AMissionReachTarget::AMissionReachTarget()
{
...
void AMissionReachTarget::BeginPlay()
{
	Super::BeginPlay();
	...
	if (ReachTrigger)
	{
		ReachTrigger->OnComponentBeginOverlap.AddDynamic(this, &AMissionReachTarget::HandleReachTriggerBeginOverlap);
	}

	if (bStartMissionOnBeginPlay)
	{
		StartMissionFromTarget();
	}
}

void AMissionReachTarget::OnConstruction(const FTransform& Transform)
{
...
EDataValidationResult AMissionReachTarget::IsDataValid(FDataValidationContext& Context) const
{
	...
	{
		Context.AddWarning(FText::FromString(TEXT("MissionName is empty; the mission can still run but will log without a title.")));
...
void AMissionReachTarget::StartMissionFromTarget()
{
...
void AMissionReachTarget::CompleteMissionFromTarget(AActor* OverlappingActor)
{
	...
	if (bRequireMissionToBeActive && !SandboxGameState->IsMissionActive())
	{
		return;
	}

	SandboxGameState->CompleteMission(CompletionText);
	UpdatePrompt();

	if (bDestroyAfterMissionComplete)
	{
		Destroy();
	}
}

void AMissionReachTarget::UpdatePrompt()
{
	if (!ObjectivePromptComponent)
	{
		return;
	...
	ASandboxGameState* SandboxGameState = GetWorld() ? GetWorld()->GetGameState<ASandboxGameState>() : nullptr;
	const bool bMissionCompleted = SandboxGameState && SandboxGameState->IsMissionCompleted();
	const FText PromptText = bMissionCompleted
		? CompletionText
		: (!ObjectivePromptText.IsEmpty() ? ObjectivePromptText : ObjectiveText);

	PromptWidget->SetPromptText(PromptText);
	PromptWidget->SetPromptVisible(bShowObjectivePrompt && !PromptText.IsEmpty());
}

bool AMissionReachTarget::IsPlayerActor(const AActor* OtherActor) const
{
	const APawn* Pawn = Cast<APawn>(OtherActor);
	return Pawn && Pawn->IsPlayerControlled();
}

void AMissionReachTarget::HandleReachTriggerBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor,
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (!bCompleteMissionOnOverlap || !OtherActor || !IsPlayerActor(OtherActor))
	{
		return;
	}

	CompleteMissionFromTarget(OtherActor);
}
